<script lang="ts">
  ...
  let { goal, onclick }: { goal: GoalUsage, onclick?: () => void } = $props();

  const color = $derived(goal.color || '#64748b');
  const percentage = $derived(Math.min(100, goal.percentage));
...
>
  ...
  <div class="space-y-1 relative z-10">
    <h3 class="text-sm font-bold text-white/90 truncate tracking-tight">{goal.name}</h3>
    <h2 class="text-xl font-black text-white tracking-tighter truncate">{formatCurrency(goal.target_amount)}</h2>
  </div>
